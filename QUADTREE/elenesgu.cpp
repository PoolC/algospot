...
void QuadTree::DoTest() {
	...
	b
	visual image of stack structure
	*/
	level++;
	string Sum;
	string parts[4];
	...
	/* 
		Let all nodes are x.
		x[i] is 'i'th x's sub-string.
		In Stack,
		x[0], x[1], x[2], x[3] is piled in ascending order.
		However, order of result tree is x[2], x[3], x[0] and x[1].
		Namely, continuous pop occurs mis-match between x and its sub-string.
		Therefore, we should pop in descending order and match x and its sub-string correctly, 
		and re-order x[i]s next for-loop.
	*/
